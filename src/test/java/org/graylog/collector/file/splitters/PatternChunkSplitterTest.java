...
public class PatternChunkSplitterTest {
            ...
            switch (++messageNum) {
                ...
                case 2:
                    assertEquals("Feb 20 17:05:18 otter GoogleSoftwareUpdateDaemon[32697]: -[KeystoneDaemon logServiceState] GoogleSoftwareUpdate daemon (1.1.0.3659) vending:\n" +
                                    ...
                                    "\t\tcom.google.Keystone.Daemon.Administration: 0 connection(s)\n",
                            chunk.getMessageString());
                    assertEquals(261, chunk.getRawMessageLength());
                    break;
                case 3:
    ...
    public void testEncodings() throws Exception {
        ...
        final FileChunkBuffer fileChunkBuffer = new FileChunkBuffer(null, buffer, 0);
        final Iterator<FileChunkMessage> iterator = splitter.splitRemaining(fileChunkBuffer, ISO_8859_1).iterator();

        assertEquals("Feb 20 17:05:18 Hällö Wörld\n", iterator.next().getMessageString());
        assertEquals("Feb 20 17:05:18 Büe\n", iterator.next().getMessageString());

        // With wrong encoding
        final ByteBuf buffer2 = Unpooled.copiedBuffer(bytes);
        final FileChunkBuffer fileChunkBuffer2 = new FileChunkBuffer(null, buffer2, 0);
        final Iterator<FileChunkMessage> iterator2 = splitter.splitRemaining(fileChunkBuffer2, UTF_8).iterator();

        assertNotEquals("Feb 20 17:05:18 Hällö Wörld\n", iterator2.next().getMessageString());
